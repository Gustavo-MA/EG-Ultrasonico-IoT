...
const char* mqtt_server = "192.168.1.105"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
...
int data = 0; // Contador
int wait = 500;  // Indica la espera cada 1 segundos para envío de mensajes MQTT
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("FCAeI/mqtt/GMA/Ultrasonico", char_array); 
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "FCAeI/mqtt/GMA/Ultrasonico") {  // En caso de recibirse mensaje en el tema codigoIoT/ejemplo/mqttin
    if(messageTemp == "true"){
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("FCAeI/mqtt/GMA/Ultrasonico"); // Esta función realiza la suscripción al tema
